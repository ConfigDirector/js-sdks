...
const textOf = (page: Page, selector: string) =>
  expect.poll(
    async () => (await page.locator(selector).textContent())?.trim(),
    {
      timeout: 10_000,
    },
  );
...
describe("ConfigDirector Nuxt SDK — Browser composables", async () => {
  ...
  it(
    "updates config values in the DOM after client-side hydration",
    { timeout: 30_000 },
    async () => {
      ...
      await textOf(page, '[data-testid="welcome"]').toBe(
        "Hello from ConfigDirector!",
      );
  ...
  it(
    "reconnects and sends the new context when updateContext is called",
    { timeout: 30_000 },
    async () => {
  ...
  it(
    ...
    async () => {
      ...
      const telemetryDone = page.waitForResponse(
        (resp) =>
          resp.url().includes("/client/telemetry/v1") &&
