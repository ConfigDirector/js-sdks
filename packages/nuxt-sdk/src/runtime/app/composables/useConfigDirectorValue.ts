import { shallowRef, readonly, onBeforeUnmount } from "vue";
...
export const useConfigDirectorValue = <T extends ConfigValueType>(
  configKey: string,
  defaultValue: T,
): { value: DeepReadonly<ShallowRef<T>>; readyStatus: Readonly<Ref<ClientStatus>>; loading: Readonly<Ref<boolean>> } => {
  ...
  return { value: readonly(configValue), readyStatus, loading };
};
