...
const sseEvent = (bundle: object): string => `data: ${JSON.stringify(bundle)}\n\n`;
...
const handleSseRequest = (bundle: object, req: IncomingMessage, res: ServerResponse): void => {
  req.resume();
  req.on("end", () => {
    res.writeHead(200, {
...
const handleTelemetryRequest = (req: IncomingMessage, res: ServerResponse): void => {
  req.resume();
  req.on("end", () => {
...
export const startMockSseServer = (): Promise<MockSseServer> => {
  return new Promise((resolve, reject) => {
    const server: Server = createServer((req: IncomingMessage, res: ServerResponse) => {
      ...
      } else if (pathname === "/server/telemetry/v1" || pathname === "/client/telemetry/v1") {
